...
-- ─────────────────────────────────────────────────────────────────────────────
-- CAFE SIGNUPS (instant café signup: form -> Telegram verify -> Stripe trial)
-- One row per /start submission on the instant path. cafe_id is generated up
-- front so a retry after a partial failure reuses the same café id and Stripe
-- idempotency key. See 2026-09-24-instant-signup.sql.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- 'provisioned' counts as open (Stripe succeeded but profile/cafe not yet
-- created) so a second form submission can't race a retry-in-progress.
...
-- contact). Nothing reads these rows except the webhook's atomic count.
...
-- signup (possession-based link, not a typed-in attempt-limited code).
